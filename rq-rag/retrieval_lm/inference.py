...
def generate_tree_of_thoughts(
    ...
    paths = [
        {"prompt": prompt, "depth": 0, "text": "", "done": False, "retrieved_index": []}
    ...
    while paths:
        ...
        for special_token in expand_on_tokens:
            ...
            if special_token != "[A_Response]":
                ...
                outputs = model.generate(
                    **inputs, return_dict_in_generate=True, do_sample=False
                )
            ...
            if special_token == "[A_Response]":
                ...
                if args.task in ["2wikimultihopqa", "hotpotqa", "musique"]:
                    if args.search_engine_type == "openai_embed":
                        evidences, top_indices = search_engine_api(
                            query_for_search,
                            corpus=raw_datas[0]["context"],
                            index=index,
                        )
                    else:
                        if args.search_engine_type == "duckduckgo":
                            ...
                            evidences, top_indices = search_engine_api(
                                query_for_search, corpus=raw_datas[0]["context"]
                            )
...
def main():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="choose whether to use huggingface for inference or use vllm",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="whether to use flash_attn, but we have to check whether it will cause much difference",
    )
    parser.add_argument(
        "--search_limit", type=int, default=1, help="whether to limit search"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="pratically usefull when inference 2wikihop, may retrieve up to 4 times",
    )
    parser.add_argument(
        "--oracle",
        default=False,
        action="store_true",
        help="it means that we are testing the upperbound of the system, where we assume all candidates are valid.",
    )
    parser.add_argument("--search_engine_type", type=str, required=True, default=None)
    parser.add_argument(
        "--rapidapi_name", type=str, default="one", help="custom rapid api "
    )
    parser.add_argument(
        "--expand_on_tokens",
        nargs="+",
        help="do not want to expand on all, we have three different special tokens",
    )
    ...
    if not args.overwrite_output_dir:
        if os.path.exists(os.path.join(args.output_path, "final_results.json")):
            ...
            logging.warning(
                f"Already done {len(tmp_results['all_preds'])} instances, continue!"
            )
    ...
    stop_words = ["[EOS]"]
    stop_words_ids = [
        tokenizer.encode(stop_word, add_special_tokens=False, return_tensors="pt")
        .squeeze(0)
        .to(model.device)
    ...
    for i in trange(0, len(input_data[start_index:]), inference_batch_size):
        ...
        if "arc" in args.task or "openbookqa" in args.task:
            ...
            golds.extend(
                [
                    row["answers"] if "output" not in row else row["output"]
                    for row in rows
                ]
            )
        ...
        if i % (2 * inference_batch_size) == 0:
            ...
            with open(
                os.path.join(args.output_path, "tmp_results.json"), "w"
            ) as outfile:
